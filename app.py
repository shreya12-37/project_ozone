...
def sendEmail():
	data = request.get_json()
	contactdetails={"fname": data['fname'],"email_id": data[email_id]}
	code, msg= sendMail(email_id,contactdetails)
	if code==200:
		d1['msg']="email sent to user"
		return jsonify(d1)
	else:
		d2['msg']="email could not be sent"
		return jsonify(d2) 
...
@app.route('/product/<redeem_coins>',methods=['get'])
def updateCoinsInUsersTable(redeem_coins):
	code,message = coinsUpdateOnPurchase(redeem_coins)
	if code==200:
		d1['msg']="coins deducted from user table"
		return jsonify(d1)
	else:
		return jsonify(d2)
